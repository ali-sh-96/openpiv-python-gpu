...
class stitch_gpu:
    ...
    **kwargs
    ...
    def __call__(self, frame_a, frame_b):
        ...
        assert isinstance(frame_a, types) and \
        ...
        assert isinstance(frame_b, types) and \
...
class StitchGPU:
    ...
    def __init__(self, frame_a_shape, frame_b_shape,
        ...
        self.zu = cp.zeros((self.y_offset, self.x_offset), dtype=self.dtype_u)
        ht_t = self.ht_a + self.y_offset - self.ht_b
        if self.case_a:
            self.zd = cp.zeros((ht_t, self.wd_b - self.overlap), dtype=self.dtype_u)
            self.fm = cp.tile(cp.linspace(1, 0, self.overlap, dtype=self.dtype_f), (self.ht_b - self.y_offset, 1))
        else:
            self.zd = cp.zeros((-ht_t, self.x_offset), dtype=self.dtype_u)
            self.fm = cp.tile(cp.linspace(1, 0, self.overlap, dtype=self.dtype_f), (self.ht_a, 1))
    ...
    def __call__(self, frame_a, frame_b):
        ...
        if self.case_a:
            fl = cp.concatenate((self.zu, frame_a[:, :self.x_offset]), axis=0)
            fr = cp.concatenate((frame_b[:, self.overlap:], self.zd), axis=0)
        else:
            fl = cp.concatenate((self.zu, frame_a[:, :self.x_offset], self.zd), axis=0)
